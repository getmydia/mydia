defmodule Mydia.Downloads.QueueTest do
  ...
  describe "select_and_add_to_client/2 — trackerless .torrent files" do
    # Production regression (1337x via Prowlarr): /download served a real
    # .torrent instead of redirecting to a magnet, and the file was generated by
    # go.torrent with no announce key at all. Transmission took it with
    # trackers=[], DHT was unreachable inside the VPN namespace, and the
    # download sat at 0 bytes forever. PR #331 had fixed the same failure for
    # magnets only.
    @info "d5:filesld6:lengthi835673131e4:pathl8:test.mkveee" <>
            "4:name8:test.mkv12:piece lengthi16384e6:pieces0:e"

    @trackerless_torrent "d7:comment28:dynamic metainfo from client" <>
                           "10:created by10:go.torrent13:creation datei1786573608e" <>
                           "4:info" <> @info <> "e"

    @tag :tmp_dir
    test "enriches the file before it reaches the client", %{tmp_dir: tmp_dir} do
      ...
      search_result = %SearchResult{
        title: "X-Men.97.S02E06.1080p.WEB-DL",
        indexer: "1337x",
      ...
      assert {:ok, _client_config, _client_id, :torrent} =
               Queue.select_and_add_to_client(search_result, [])

      assert [written] = File.ls!(watch)
      body = File.read!(Path.join(watch, written))

      assert String.contains?(body, "8:announce")
      assert String.contains?(body, "13:announce-list")
      assert String.contains?(body, "udp://tracker.opentrackr.org:1337/announce")

      # The info hash is what the client dedupes and reports on, so it must
