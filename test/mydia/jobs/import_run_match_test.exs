defmodule Mydia.Jobs.ImportRunMatchTest do
  ...
  test "writes a candidate for every outstanding file, leaving none outstanding", %{
    run: run,
    library_path: lp,
    config: config,
    counter: counter
    ...
    assert Repo.aggregate(ImportCandidate, :count) == 5
  ...
  test "review mode promotes nothing from external matches", %{
    run: run,
    library_path: lp,
    config: config,
    counter: counter
  } do
    # Review mode never promotes from an external provider match: that is the
    # judgement call the human is here to make. This fixture library's Bypass
    # stub returns no results at all, so every candidate ends up a recorded
    # failure (no provider match), which is also the "review mode never
    # promotes" case with the least room for the two to be confused with each
    # other.
    :ok = ImportRunJob.run_match_phase(Library.get_import_run(run.id), config: config)
    ...
    # Every file got a candidate instead, none of them matched -- the inbox
    # handoff.
    candidates = Repo.all(ImportCandidate)
    assert length(candidates) == 5
    assert Enum.all?(candidates, &is_nil(&1.provider_id))
    assert Enum.all?(candidates, &(&1.last_error == "no_match"))
  ...
  test "loose files in root match independently against metadata relay", %{
    ...
    matrix_candidate = ImportCandidates.get_by_path(lp.id, "The.Matrix.1999.1080p.mkv")
    inception_candidate = ImportCandidates.get_by_path(lp.id, "Inception.2010.1080p.mkv")
  ...
  test "keeps candidates a chunk already committed when a stop lands mid-match, and a resumed run finishes the rest",
    ...
    total_outstanding = ImportCandidates.count_outstanding(lp.id)
    assert total_outstanding == 55
    ...
    remaining = ImportCandidates.count_outstanding(lp.id)
    ...
    assert partial_matched < total_outstanding
    assert remaining == total_outstanding - partial_matched
    ...
    # Every candidate the first (interrupted) run resolved is untouched by the
    # resume: only the remainder got matched again.
  ...
  describe "the progress contract backstop" do
    test "fails the run instead of reporting :ok when a candidate is left neither matched, retried, dismissed, nor promoted",
         %{run: run, library_path: lp} do
      # InvalidCandidateMatcher's confidence is outside
      # ImportCandidate.changeset/2's valid range, so
      # Mydia.ImportCandidates.upsert/1 genuinely rejects the write and
      # FileIngest.ingest/3 returns {:error, {:candidate_write_failed, _}}
      # for every candidate in this run's single chunk -- each one left with
      # no match, no retry timestamp, no dismissal, and no promotion, which is
      # exactly the progress-contract violation verify_match_phase_complete/2
      # exists to catch once the keyset walk (which never re-visits a chunk's
      # ids once it has advanced past them) reports the phase done regardless.
      log =
        ExUnit.CaptureLog.capture_log(fn ->
          assert {:error, {:candidates_outstanding, message}} =
                   ImportRunJob.run_match_phase(Library.get_import_run(run.id),
      ...
      assert log =~ "count=5"
      assert log =~ "candidate_ids="

      # Not just a message: the candidates really are still there, unmatched,
      # so the run's failure reflects real stuck state rather than a check
      # that fired on stale information.
      assert ImportCandidates.count_outstanding(lp.id) == 5
    end
  end
end
