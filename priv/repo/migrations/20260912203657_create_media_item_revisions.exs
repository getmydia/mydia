defmodule Mydia.Repo.Migrations.CreateMediaItemRevisions do
  ...
  defp create_revision_table do
    if postgres?() do
      ...
      CREATE TABLE media_item_revisions (
        ...
        changed_at TIMESTAMP(6) WITHOUT TIME ZONE NOT NULL DEFAULT timezone('UTC', now())
      )
      """)
    else
      execute("""
  ...
  defp install_postgres_functions do
    ...
    VALUES (p_revision, p_media_item_id, p_deleted, timezone('UTC', now()))
  ...
  defp install_postgres_triggers do
    ...
    # row (and therefore the association to them) still exists.
    postgres_trigger(
  ...
  defp postgres_trigger(name, event, when_clause, body) do
    execute("""
