...
Read the upgrade notes before pulling this. There are 18 migrations (all additive or column conversions), download audio and subtitle preferences are separated from playback defaults, and movies and TV shows can now share external IDs.
...
- `ENABLE_PLAYER=false` turns off everything the player needs: p2p node, HLS, transcoding, hwaccel probe, and HTTPS listener (#778)
...
- Daily upgrade sweep replaces files missing preferred audio while leaving untagged audio untouched (#823, #824)
...
Back up first. Eighteen migrations, all additive or column conversions.
...
3. **Daily upgrade sweep replaces missing audio languages.** Files lacking both the preferred download audio language and English are queued for upgrade; untagged files are not touched (#823).
4. **`ENABLE_PLAYER=false` shuts down the player stack.** This turns off the p2p node, HLS streaming, transcoding, hardware acceleration probing, intro detection, and the HTTPS listener (#778).
