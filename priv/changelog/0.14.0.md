...
- Breaking change for Library API pollers: `mediaItems(updatedSince:)` and its timestamp cursors are replaced by `mediaItemChanges(after:)`, a revision-ordered change feed that reports deletions as tombstones. Old cursors are invalid, so a client holding one has to start a fresh snapshot. `MediaItem.updatedAt` now reports the item's aggregate change time, which on upgrade is the migration instant for every existing item until it next changes
...
- On iOS and Android phones, episode-to-episode playback stays landscape instead of rotating back to portrait between episodes.
