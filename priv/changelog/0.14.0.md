...
- Breaking change for Library API pollers: `mediaItems(updatedSince:)` and its timestamp cursors are replaced by `mediaItemChanges(after:)`, a revision-ordered change feed that reports deletions as tombstones. Old cursors are invalid, so a client holding one has to start a fresh snapshot
