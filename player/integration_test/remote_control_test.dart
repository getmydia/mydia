...
//
// Coverage against the plan's 7 steps, all seven steps run on every CI pass:
...
void main() {
  ...
  testWidgets('one player drives another end to end', (tester) async {
    ...
    // A dials B by bare node ID, which normally resolves through n0's DNS.
    // The E2E container has no business depending on that, so hand A the
    // address directly: the code path under test is unchanged, only the
    // lookup that answers it is local.
    final bNodeAddr = await bP2p.getNodeAddr();
    expect(bNodeAddr, isNotNull, reason: 'Player B has no node address');
    await container.read(p2pServiceProvider).addAddressHint(bNodeAddr!);
