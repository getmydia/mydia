...
class BrowseScaffold extends ConsumerWidget {
  ...
  /// Rendered between the title and the cast button [WindowTitleRow] always
  /// appends last.
  ///
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // The whole body sits under `removeBand`: this screen's own
    // `WindowTitleRow` (built by `_buildAppBar`) draws into the band itself,
    // so the ambient `MediaQuery.padding.top` this subtree sees must no
    // longer contain it. Read MediaQuery inside this builder, above the
    // `Scaffold`: inside the body of a `Scaffold(extendBodyBehindAppBar:
    // true)` Flutter rewrites `padding.top` to the app bar's own bottom edge
    // (see `_BodyBuilder` in material/scaffold.dart), so a descendant that
    // reads it and adds the bar height again counts the bar twice. That was
    // `LibraryScreen`'s original bug and the reason this math lives at the
    // top of the builder rather than inside `Scaffold.body`.
    return WindowChromeInsets.removeBand(
      child: Builder(
        builder: (context) {
          final barHeight = WindowTitleRow.heightOf(context) +
              (secondRow == null ? 0.0 : secondRowHeight);
          ...
          return Scaffold(
            ...
            appBar: _buildAppBar(context, barHeight),
            body: Stack(
              children: [
                if (refresh == null)
                  content
    ...
    return PreferredSize(
      ...
      child: GlassSurface.appBar(
        ...
        child: Column(
          ...
          children: [
            WindowTitleRow(
              // A pushed screen gets back; a shell destination gets the
              // drawer. Search is the case that forces this: every other
              // browse screen reaches it with `context.push('/search')`, so
              // it sits on top of the shell navigator, and a hamburger there
              // goes sideways rather than back. Desktop had no leading
              // control at all, which made the push a one-way trip.
              //
              // `Navigator`, not go_router's `context.canPop()`: inside the
              // shell route this resolves the shell navigator, which is
              // exactly what a pushed screen sits on, and it does not
              // require a GoRouter ancestor, so this widget stays pumpable
              // in a plain widget test.
              ...
              // The label ellipsises: this bar uses `headlineSmall` to match
              // LibraryScreen, which is larger than the `titleLarge` the old
              // mobile bars used. "Continue Watching" at that size overflows
              // a narrow phone bar once the drawer and cast buttons take
              // their share.
              title: Padding(
