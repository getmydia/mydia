...
class ChromeVisibility extends StatefulWidget {
  ...
  /// (`setWindowButtonsHidden` from `core/window/window_buttons_bridge.dart`).
  final void Function(bool hidden)? onWindowButtonsHidden;
...
class _ChromeVisibilityState extends State<ChromeVisibility>
  ...
  void initState() {
    ...
    // already be fullscreen at mount time — see shouldCallNativeButtonBridge'
  ...
  void Function(bool hidden) get _setWindowButtonsHidden =>
      widget.onWindowButtonsHidden ?? setWindowButtonsHidden;
...
/// Keeps toasts above the control panel while it is shown.
///
/// The chrome hides by fading and sliding behind an `IgnorePointer` while
/// staying mounted with its tickers running, so `ToastObstruction`'s
/// `TickerMode` check cannot see it go. This follows the same show/hide
/// animation [ChromeSlide] reads and withdraws the claim the moment a hide
/// starts, so the toast moves alongside the panel rather than after it.
/// With no [ChromeAnimation] above it, it always counts.
class ChromeToastObstruction extends StatefulWidget {
  const ChromeToastObstruction({super.key, required this.child});

  final Widget child;

  @override
  State<ChromeToastObstruction> createState() => _ChromeToastObstructionState();
}

class _ChromeToastObstructionState extends State<ChromeToastObstruction> {
  ...
  void _handleStatus(AnimationStatus status) => setState(() {});

  bool get _shown => _animation?.status.isForwardOrCompleted ?? true;

  @override
  Widget build(BuildContext context) => ToastObstruction(
...
class PlaybackChrome extends StatefulWidget {
  ...
  /// Where the chrome sends the subtitle lift while its panel shows. Null
  /// leaves subtitles where media_kit puts them, which is under the panel.
  final SubtitleLift? subtitleLift;
...
class _PlaybackChromeState extends State<PlaybackChrome> {
  ...
  /// The chrome's own root box. It fills the same `Video` stack as
  /// `SubtitleView`, so its bottom edge is the subtitles' bottom edge.
  RenderBox? _subtitleReference() {
