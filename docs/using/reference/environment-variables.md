...
| `MYDIA_METRICS_ENABLED` | Serves Prometheus metrics at `/metrics`: memory, request latency, background jobs, library size, downloads and active streams. The endpoint has no authentication, so keep it off your public reverse proxy, including the HTTPS player port when the player is enabled. Accepts `true`, `1`, `yes`, `on`. See [Monitoring with Prometheus](../how-to/monitoring-with-prometheus.md) | `false` |
