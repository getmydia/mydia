...
# --enforce-lockfile like every other CI `pub get`: player/Dockerfile.test
# installs the SDK from player/.fvmrc, so the lock resolves as committed.
flutter pub get --enforce-lockfile >/tmp/flutter-pub-get.log 2>&1 || {
